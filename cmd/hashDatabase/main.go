...
func subScribeNewHead(client *ethclient.Client, hashDb *db.LDBDatabase, ctx context.Context) {
	...
	for {
		...
		case header := <-headerCh:
			...
			//todo 由于block的交易数据太大，获取、存储一条交易时间比较久。。。
			// block, err := client.BlockByNumber(ctx, header.Number)
			// if err != nil {
			// 	fmt.Printf("GetBlock %s failed: %s\n", header.Number.String(), err)
			// }
			// for _, tx := range block.Transactions() {
			// 	data := tx.Data()
			// 	dataHash := common.BytesToHash(data[20:])
			// 	//fmt.Println(dataHash.String(), tx.Hash().String())
			// 	err = hashDb.InsertHash(dataHash, tx.Hash())
			// 	if err != nil {
			// 		fmt.Printf("InsertHash failed: hash: %s,txId:%s\n", dataHash.String(), tx.Hash().String())
			// 	}
			// }
		}
	}
